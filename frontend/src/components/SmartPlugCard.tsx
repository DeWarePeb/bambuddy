...
import { Plug, Power, PowerOff, Loader2, Trash2, Settings2, Thermometer, Clock, Wifi, WifiOff, Edit2, Bell, Calendar, LayoutGrid, ExternalLink, Home, Radio, Eye, Globe, Zap } from 'lucide-react';
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              <div className={`p-2 rounded-lg flex-shrink-0 ${
                ...
                ) : plug.plug_type === 'moonraker' ? (
                  /* Voron patch series (C3): a Moonraker [power] device. */
                  <Zap className={`w-5 h-5 ${isReachable ? (isOn ? 'text-bambu-green' : 'text-bambu-gray') : 'text-red-600 dark:text-red-400'}`} />
                ) : (
              ...
              <div className="min-w-0">
                ...
                <p
                  ...
                  title={plug.plug_type === 'mqtt' ? plug.mqtt_topic ?? undefined : plug.plug_type === 'homeassistant' ? plug.ha_entity_id ?? undefined : plug.plug_type === 'rest' ? plug.rest_on_url ?? plug.rest_off_url ?? undefined : plug.plug_type === 'moonraker' ? plug.moonraker_device ?? undefined : plug.ip_address ?? undefined}
                >
                  {plug.plug_type === 'mqtt' ? plug.mqtt_topic : plug.plug_type === 'homeassistant' ? plug.ha_entity_id : plug.plug_type === 'rest' ? (plug.rest_on_url || plug.rest_off_url) : plug.plug_type === 'moonraker' ? plug.moonraker_device : plug.ip_address}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-end gap-1 flex-shrink-0">
              {statusLoading ? (
                ...
                <div className="flex items-center gap-1 text-sm">
                  <span className="px-1 py-0.5 bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400 text-[10px] font-medium rounded">Moonraker</span>
                  <span className={isReachable ? (isOn ? 'text-status-ok' : 'text-bambu-gray') : 'text-status-error'}>
                    {isReachable ? (status?.state || '?') : t('smartPlugs.offline')}
                  </span>
                </div>
