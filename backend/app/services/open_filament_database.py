"""Open Filament Database (OFDB) client and spool-prefill normalisation.

The upstream API (https://api.openfilamentdatabase.org) is a static JSON tree:
brands -> brand (materials) -> material (filaments) -> filament (variants) -> variant (sizes).
The browser never talks to it directly (CORS, and the URL should be a single
place to change); the spool form goes through the routes in
``backend/app/api/routes/open_filament_database.py`` which call this client.

Every public method returns a plain dict shaped for the frontend. The
``spool_prefill`` block on filament/variant responses uses Bambuddy's own spool
field names so the form can copy it over without knowing the OFDB hierarchy.
"""
...
# Stored in Spool.data_origin (String(20)) — exactly 20 characters, do not lengthen.
OFDB_SOURCE = "openfilamentdatabase"

# Which slicer's preset block to read the preset id / profile name from. Bambu
# Studio first because that is what reaches the printer; Orca ids are the same
# GF codes in practice.
_SLICER_PREFERENCE = ("bambustudio", "bambu_studio", "orcaslicer", "prusaslicer", "elegooslicer")
...
def _clean_segment(value: str) -> str:
    """Turn a user-supplied slug into one safe URL path segment."""
    cleaned = value.strip().strip("/")
    if not cleaned or cleaned in {".", ".."} or "/" in cleaned:
        raise ValueError("path segment must not be empty or contain '/'")
    return quote(cleaned, safe="._-")


def _rgba_from_hex(color_hex: str | None) -> str | None:
    """'#1a2B3c' -> '1A2B3CFF'. Anything that is not 6 hex digits yields None."""
    if not color_hex:
        return None
...
def _derive_subtype(material: str | None, filament_name: str | None) -> str | None:
    """'PLA Basic' with material PLA -> 'Basic'; 'PLA' -> 'PLA'; 'PolyTerra PLA' -> 'PolyTerra'."""
...
def _first_preferred_size(sizes: list[dict[str, Any]]) -> dict[str, Any] | None:
    """Pick the size to prefill from: current, on a spool (not a refill), 1 kg if there is one."""
    dicts = [s for s in sizes if isinstance(s, dict)]
    active = [s for s in dicts if not s.get("discontinued")] or dicts
    candidates = [s for s in active if not s.get("spool_refill")] or active
    if not candidates:
        return None
...
class OpenFilamentDatabaseClient:
    def __init__(self, *, base_url: str = OFDB_BASE_URL, timeout: float = 10.0, transport: Any = None):
    ...
    async def _get_json(self, path: str) -> dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(
                timeout=self.timeout, headers=self.headers, follow_redirects=True, transport=self.transport
            ) as client:
        ...
        # OFDB is a static site: a missing entry is an HTML 404 page, never JSON.
    ...
    async def get_brands(self) -> dict[str, Any]:
        ...
        brands = self._list(payload, "brands")
        return {
            "source": OFDB_SOURCE,
    ...
    async def get_brand(self, brand_slug: str) -> dict[str, Any]:
        ...
        return {
            ...
            "materials": self._list(payload, "materials"),
        }
    ...
    async def get_material(self, brand_slug: str, material: str) -> dict[str, Any]:
        ...
        return {
            ...
            "filaments": self._list(payload, "filaments"),
        }

    async def search_filaments(self, brand_slug: str, material: str, query: str | None = None) -> dict[str, Any]:
        """Filter one brand/material's filament families by name. OFDB has no global text search."""
        ...
        if q:
            filaments = [
                ...
                and (
                    q in str(filament.get("name", "")).lower()
    ...
    async def get_filament(self, brand_slug: str, material: str, filament_slug: str) -> dict[str, Any]:
        ...
        slicer = _pick_slicer_settings(slicer_settings)
        ...
        return {
            ...
            "slicer_settings": slicer_settings if isinstance(slicer_settings, dict) else {},
            "preferred_slicer_setting": slicer,
            "variants": self._list(payload, "variants"),
            "spool_prefill": {
                "material": material_name,
                "subtype": _derive_subtype(material_name, filament_name),
                "nozzle_temp_min": _number(payload.get("min_print_temperature")),
                "nozzle_temp_max": _number(payload.get("max_print_temperature")),
    ...
    async def get_variant(
        ...
        sizes = self._list(payload, "sizes")
        selected_size = _first_preferred_size(sizes)
        color_hex = payload.get("color_hex")
        spool_prefill = dict(filament_payload["spool_prefill"])
        spool_prefill.update(
            {
                "brand": brand_name,
                "color_name": payload.get("name"),
                "rgba": _rgba_from_hex(color_hex),
                "label_weight": (_number(selected_size.get("filament_weight")) if selected_size else None) or 1000,
                "data_origin": OFDB_SOURCE,
            }
        )
        # Not every OFDB size lists the empty spool weight; only prefill when it does.
        core_weight = _number(selected_size.get("empty_spool_weight")) if selected_size else None
        if core_weight:
            spool_prefill["core_weight"] = core_weight
        return {
            "source": OFDB_SOURCE,
            "brand": {
