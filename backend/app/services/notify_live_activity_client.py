...
Notify (getnotifyapp.com) exposes a small REST API per device: one endpoint
for normal pushes and one for Live Activities (the Dynamic Island / lock
screen tile). This client only covers the Live Activity endpoint; normal
pushes go through ``NotificationService._send_notify``.
"""
...
DEFAULT_NOTIFY_BASE_URL = "https://push.getnotifyapp.com"
...
class NotifyLiveActivityClient:
    ...
    def __init__(self, config: dict[str, Any], *, http_client: Any):
        ...
        self.base_url = str(config.get("base_url") or DEFAULT_NOTIFY_BASE_URL).strip().rstrip("/")
        ...
        # The gateway URL is user-supplied config; apply the same LAN-service
        # policy the other providers use so a provider cannot be pointed at
        # cloud-metadata or non-HTTP destinations.
        from backend.app.api.routes._url_safety import assert_safe_lan_service_url

        try:
            assert_safe_lan_service_url(self.base_url, label="Notify gateway URL")
        except ValueError as exc:
            raise NotifyLiveActivityError(str(exc), retryable=False) from exc
    ...
    async def end(self, activity_id: str, content: dict[str, Any] | None = None, *, keep_for_seconds: int = 0) -> None:
        """End an existing Live Activity, optionally keeping the final tile for a while."""
    ...
    def _raise_for_error(self, response: Any) -> None:
        ...
        raise NotifyLiveActivityError(
            f"Notify Live Activity request failed with HTTP {status_code}: {self._redact(text)}",
    ...
    def _redact(self, value: str) -> str:
        if self.device_token:
            return value.replace(self.device_token, "[REDACTED]")
        return value
