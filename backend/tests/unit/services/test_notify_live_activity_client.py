"""Tests for the Notify Live Activity HTTP client (voron B9)."""
...
def test_missing_credentials_and_unsafe_gateway_are_rejected(mock_client):
    with pytest.raises(NotifyLiveActivityError):
        NotifyLiveActivityClient({"device_id": "", "device_token": "x"}, http_client=mock_client)

    with pytest.raises(NotifyLiveActivityError):
        NotifyLiveActivityClient(
            {"device_id": "d", "device_token": "t", "base_url": "ftp://push.example"}, http_client=mock_client
        )
